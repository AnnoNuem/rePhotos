...
def build_regular_mesh(width, height, grid_size):
    ...
    quads = np.zeros((grid_points.shape[0] - x.size - y.size + 1, 4), 
        dtype=np.int64) 
    quads[:, 0] = np.reshape(np.tile(np.arange(0, m - 1, 1),[n-1, 1]) +  
        m* np.tile(np.arange(0, n - 1, 1),[m - 1, 1]).T, [-1, 1]).T
    quads[:, 1] = np.reshape(np.tile(np.arange(0, m - 1, 1),[n-1, 1]) +  
        m* np.tile(np.arange(1, n,     1),[m - 1, 1]).T, [-1, 1]).T
    quads[:, 2] = np.reshape(np.tile(np.arange(1, m,     1),[n-1, 1]) +  
        m* np.tile(np.arange(1, n,     1),[m - 1, 1]).T, [-1, 1]).T
    quads[:, 3] = np.reshape(np.tile(np.arange(1, m,     1),[n-1, 1]) +
        m* np.tile(np.arange(0, n - 1, 1),[m - 1, 1]).T, [-1, 1]).T
...
def sample_lines(src_lines, dst_lines, sample_rate):
    ...
    assert len(src_lines) == len(dst_lines), \
        "Different number of lines per image."
    assert len(src_lines) > 0, "No lines."

    #TODO switch tp src dst instead dst src but matlab implemenation is like this
    line_pairs = np.concatenate((np.array(dst_lines,dtype=np.double), 
        np.array(src_lines)), axis=1)

    line_src = line_pairs[:, 0::2] + 1j* line_pairs[:, 1::2]

    fGenWt = lambda n: np.array((np.arange(n-1, -1, -1), 
        np.arange(0, n))).T/(n-1)

    fNSampleAdaptive = lambda pq: max(2, 
        np.ceil(sample_rate * abs(pq[0] -pq[1])))

    pts = [ np.dot(fGenWt(fNSampleAdaptive(line_src[i,0:2])), 
        line_src[i,:].reshape((2,-1)).T) for i in range(0,line_src.shape[0])]
    p1 =  pts[0][:, 0]
    p2 = []
    p2.append(pts[0][:, 1])
...
def bilinear_point_in_quad_mesh(pts, X, P, qmSize):
    ...
    qij = np.array((np.ceil((pts[0, :] + eps - bbox[0, 0]) * (qmSize[1] - 1)
          / peak_dist[0]), 
          np.ceil((pts[1, :] + eps - bbox[0, 1]) * (qmSize[0] - 1) 
          / peak_dist[1])))
    ...
    Ascr = csc_matrix((np.array(((1 - wx) * (1 - wy), wx * (1 - wy),  wx * wy, 
        (1 - wx) * wy )).T.flatten(), (np.tile((np.arange(0, npts , 1)), 
        [4,1]).T.flatten(),  P[q, :].flatten())), shape=(npts, nx))
...
def deform_aaap(x, Asrc, pdst, L, line_constraint_type):
    ...
    if line_constraint_type > 0:
        ...
        b[nv * 2:] = d_qr
        y = spsolve(bmat([[Lr * 2, C_qr_s.T], [C_qr_s, 
            csc_matrix((d_qr.size, d_qr.size))]]).tocsc(), b)
